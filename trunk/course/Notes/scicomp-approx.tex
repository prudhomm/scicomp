...
\begin{frame}{Construction}
  ...
  \begin{block}{Remark}
    ...
    \item[GOOD] Hierarchical basis
    \item[GOOD] good (constsant) mass matrix conditioning as polynomial degree increases
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Dubiner Basis}
  ...
  \begin{equation}
    ...
    \right.
  \end{equation}
Take $\alpha = \beta = 0$ in the Jacobi polynomials
  definition. we denote $L_k$ the $k$-th Legendre
  polynomial. $(L_k)_{k=1,...,N^d}$ form a basis of $\QS{N}(\GQ{d})$.
  
  \begin{figure}[H]
    \centering
    \subfigure[Legendre]{\includegraphics[width=\figwidth\textwidth]{../../../../../../figures/Dubiner_5}}
...
\begin{frame}[containsverbatim]{C++}
  \begin{lstlisting}
    ...
    class PointSet 
    {
      ...
      protected:
      ublas::matrix<double,row_major> M_pts;
    };
    template<class Convex,typename T>
...
\begin{frame}[label=pset]{Polynomials and Polynomial Sets}
  ...
  \begin{block}{}
    ...
    \item \alert{Hierarchical Basis}
      ...
      \end{itemize}
    \end{itemize}
    \hfill\hyperlink{psetlisting<1>}{\beamergotobutton{\cpp}}
  \end{block}
  %% \hfill\hyperlink{psetlisting<1>}{\beamergotobutton{\cpp$\triangleright$}}
  \note{Les propriétés mathématiques sont ici \alert{essentielles}}
\end{frame}

\begin{frame}[label=psetops]{Polynomiales Operations}
  \begin{block}{Modal/Nodal Mapping }
    \begin{itemize}
    \item Given $\{x_j\}_{j=1,...,\mathrm{dim} \Pk}$ a point set in $K$
    \item Given $V = (\phi_i(x_j))$  Vandermonde matrix (modal/nodal coefficient mapping)
    \item Given $\Hat{p} \in \mathbb{R}^{\mathrm{dim} \Pk}$, such that $\Hat{p} = \mathcal{I}(p)\, V$
    \end{itemize}
    \centerline{\alert{We look for $p$ (that is determine its  coefficients $\mathcal{I}(p)$)}}
  \end{block}
  \begin{example}{}
    Given $D_l$ such that  $(D_l)_{i,j}= (\frac{\partial \phi_i}{\partial x_l})(x_j)$
    \begin{itemize}
    \item $\partial_l\ \Hat{p} = \mathcal{I}( \partial_l\ p )\ D_l$, where $\partial_l\ \Hat{p}$ derivative of $p$ in $\{x_j\}$
    \item $\mathcal{I}(\partial_l\ p) = \partial_l\ \Hat{p}\ D_l^{-1}$
    \end{itemize}
  \end{example}
  \begin{alertblock}{Features        \hfill\hyperlink{psetopslisting<1>}{\beamergotobutton{\cpp}}
    }
    Any order differentiation, Gradient, Divergence, Curl, $L_2$ projection
  \end{alertblock}
\end{frame}

%%\frame[containsverbatim,label=psetlisting]{
%\frame[containsverbatim]{
\begin{frame}[containsverbatim,label=psetlisting]{Polynomial and Polynomialset}
...
\end{lstlisting}
  ...
  \begin{block}{}
    \begin{lstlisting}[mathescape,basicstyle=\tiny\ttfamily]
      ...
      OrthonormalPolynomialSet<2,3,Scalar> ScalarDubiner;
      // Vectorial polynomial set
      OrthonormalPolynomialSet<2,3,Vectorial> VectorialDubiner;
    \end{lstlisting}
  \end{block}
\end{frame}

\begin{frame}[containsverbatim,label=psetopslisting]{Polynomials Operations}
  
\begin{lstlisting}[mathescape,basicstyle=\tiny\ttfamily]
// Divergence $\nabla \cdot$
template<typename Poly>
PolynomialSet<Poly, Scalar>
div( PolynomialSet<Poly, Vectorial> const& p )
{
  const int nComponents =
   PolynomialSet<Poly, Vectorial>::nComponents;
  ...
  for ( int i = 0; i <nComponents; ++i )  {
   ...
   return PolynomialSet<Poly, Scalar>( Poly(), __div );
}
...
project( Pset const& pset, PExpr const& expr, IM const& im )
{
  ...
  return Polynomial<Pset, Scalar>(pset, prod( psetq, element_prod( exprq, im.weights() ) );  );
}
\end{lstlisting}
\end{frame}
...
\begin{frame}[containsverbatim]{Polynomial Operation II}
  ...
  template<typename P, typename PS,
   template<class, class> class Poly1,
   template<class, class> class Poly2 >
  PolynomialSet<P, PS>
  unite( Poly1<P, PS> const& pset1,
         Poly2<P, PS> const& pset2 )
  {
  ...
  \end{lstlisting}
     ...
     \begin{column}{.5\textwidth}
       \begin{block}{Base orthonormale de $\Hat{P} \subset P$}
         \begin{enumerate}
         \item Construire la famille de polynômes $\mathbb{P}$ résultant
           de l'union de $\mathbb{P}^a$ et $\mathbb{P}^b$
...
\begin{frame}[label=fset]{Functionals and Functional Sets}
  ...
  \begin{block}{Functional $\sigma : \Pk \longrightarrow \mathbb{R}$}
    ...
    \end{itemize}
    \hfill\hyperlink{fsetlisting<1>}{\beamergotobutton{\cpp}}
  \end{block}
\end{frame}
...
\begin{frame}[label=fsetops]{Some Functionals}
  \begin{block}{Fonctional $\sigma : \mathbb{P}(K) \longrightarrow \mathbb{R}$}
    ...
    \item ...
    \end{itemize}
    \hfill\hyperlink{fsetopslisting<1>}{\beamergotobutton{\cpp}}
  \end{block}
\end{frame}

%% fset
\begin{frame}[fragile=singleslide,label=fsetlisting]{Functionals and Functional Sets}
\begin{lstlisting}[mathescape,basicstyle=\tiny\ttfamily]
// $\sigma$
...
\end{lstlisting}
    \hfill\hyperlink{fset<1>}{\beamerreturnbutton{$\triangleleft$}}
\end{frame}

%% fsetops
\begin{frame}[fragile=singleslide,label=fsetopslisting]{Some Functionnals}
\begin{lstlisting}[mathescape,basicstyle=\tiny\ttfamily]
...
class IntegralMoment
 : public Functional<Space>
{
 IntegralMoment( Space const& p, Poly const& q )
 : super( p )
 {
   this->setCoefficient(
...
\end{lstlisting}
  \hfill\hyperlink{fsetops<1>}{\beamerreturnbutton{$\triangleleft$}}
\end{frame}

\subsection{Applications}
\label{sec:applications}

\begin{frame}[label=apps,containsverbatim]{FE construction}
  \begin{block}{}
    \begin{itemize}
...
\begin{frame}{$D=\{ p \in [\Pk]^d,\ \mathrm{div}(p) = 0\}$}
  \begin{figure}
    ...
    \label{fig:1}
  \end{figure}
  \hfill\hyperlink{apps<1>}{\beamerreturnbutton{$\triangleleft$}}
\end{frame}
...
\begin{frame}{$(K,\ \mathbb{P}_k,\ \{\sigma_{x_i}\})$, $(K,\ [\mathbb{P}_k]^d,\ \{\sigma_{x_i}\})$}
  \begin{figure}
    ...
    \caption{Scalar Lagrange element of order 1 (top) \\
      Some Lagrange polynomials of degree 3 (bottom) \hfill\hyperlink{apps<2>}{\beamerreturnbutton{$\triangleleft$}}}
  \end{figure}
  \hfill\hyperlink{apps<1>}{\beamerreturnbutton{$\triangleleft$}}
\end{frame}
...
\end{frame}
 ...
 \begin{frame}{Problème 2D sur simplex}
   \only<1>{
     \begin{columns}
       \begin{column}[c]{.4\textwidth}
 	{\scriptsize
 ...
 %	 \movie[externalviewer=kaffeine]{\includegraphics[width=\textwidth]{../../../../../../figures/Dubiner_3.jpg}}{../../../../../../figures/Dubiner_5.jpg}
 %          \caption{\scriptsize Base de Dubiner}
 	...
 	\begin{figure}
          \centering
 	 \movie[externalviewer=kaffeine]{\includegraphics[width=50mm]{../../../../../../figures/Sol_2D.png}}{../../../../../../figures/Sol_2D.png}
         \end{figure}
 	\end{itemize}
 	}
       \end{column}
   ...
   \only<2>{
 	...
 	\item Convergence :
         \begin{figure}
 %         \centering
 	\movie[externalviewer=kaffeine]{\hspace{-13mm}\includegraphics[width=7cm]{../../../../../../figures/Error_2D.png}}{../../../../../../figures/Error_2D.png}
         \end{figure}
 	$$
 	...
 	\begin{figure}
 %         \centering
 	\movie[externalviewer=kaffeine]{\hspace{-10mm}\includegraphics[width=7cm]{../../../../../../figures/Cond_2D.png}}{../../../../../../figures/Cond_2D.png}
 	\end{figure}
 	$$
 ...
 \begin{frame}{Problème 3D sur simplex}
   \only<1>{
     \begin{columns}
       \begin{column}[c]{.4\textwidth}
 	{\scriptsize
 ...
 %	 \movie[externalviewer=kaffeine]{\includegraphics[width=50mm]{../../../../../../figures/Sol_3D.png}}{../../../../../../figures/Sol_3D.png}
   ...
   \only<2>{
 	...
 	\item Convergence :
         \begin{figure}
 	\movie[externalviewer=kaffeine]{\hspace{-13mm}\includegraphics[width=7cm]{../../../../../../figures/Error_3D.png}}{../../../../../../figures/Error_3D.png}
         \end{figure}
 	$$
 	...
 	\begin{figure}
 %         \centering
 	\movie[externalviewer=kaffeine]{\hspace{-10mm}\includegraphics[width=7cm]{../../../../../../figures/Cond_3D.png}}{../../../../../../figures/Cond_3D.png}
 	\end{figure}
 	$$
