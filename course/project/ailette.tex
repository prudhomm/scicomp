...
\title{Quelques détails sur le projet de calcul scientifique. Masters
  IMSN, M2R mathématiques appliquées et MSSPI \\
\texttt{http://code.google.com/p/scicomp/}}
\author{M. Ismail et C. Prud'homme}

\begin{document}
\maketitle

\centerline{\large Toutes les questions doivent être envoyées à
  \texttt{scicomp-dev@googlegroups.com}} 
\hspace*{.5cm}
...
dissipation de la chaleur dans un élément de radiateur (figure
\ref{fig:radiateur}).
...
\begin{figure}[!h]
  \centering
  \includegraphics[scale=0.4]{radiateur}
  \caption{Élément de radiateur}
  \label{fig:radiateur}
\end{figure}
...
l'ailette (figure \ref{fig:sectTrans})
$$
...
\begin{figure}[!h]
  \centering
  \includegraphics[scale=0.3]{ailetteMod2D}
  \caption{Section transversale de l'ailette}
  \label{fig:sectTrans}
\end{figure}
...
frontière $(y=0)$ (figure \ref{fig:sectTransSym}).

\begin{figure}[!h]
  \centering
  \includegraphics[scale=0.3]{ailetteMod2DSym}
  \caption{Domaine de calcul}
  \label{fig:sectTransSym}
\end{figure}
...
\section{Exemple de discrétisation en espace. Éléments finis {\it P1}}
...
Pour réaliser ce projet, on propose d'utiliser la librairie \texttt{Life}.
Le point de départ sera l'exemple \texttt{laplacien.cpp} fourni avec le
tutoriel (voir \texttt{/usr/ljk/share/doc/life} sur \texttt{idbull}).
...
\bibitem{danaila:03}
  ...
  \newblock {\em Simulation numérique en C++}.
  \newblock Dunod, Paris, 2003.
  
\bibitem{prudhomm:06}
  {C. Prud'homme}.
  \newblock{\em Life: Overview of a unified C++ implementation of the finite and
    spectral element methods in 1d, 2d and 3d}.
  \newblock{In Workshop On State-Of-The-Art In Scientific
    And Parallel Computing, Lecture Notes in Computer Science, page
    10}.
